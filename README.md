...
indexed into a local, per-project index under `.claude/`.
...
- Local index health (exists, size, openable, file/symbol counts, leftover
  per-process index files from dead sessions)
- Parser coverage for the languages detected in the repo
...
This repository hosts crewkit CLI releases, documentation, and the issue
tracker. Found a bug or have a feature request? See
[CONTRIBUTING.md](CONTRIBUTING.md) — bug reports with `crewkit lsp doctor`
or `--debug` output are especially useful.

## License

The crewkit CLI is proprietary software distributed as native binaries. Your
use of the CLI and the crewkit platform is governed by the
[Terms of Service](https://crewkit.io/terms).
